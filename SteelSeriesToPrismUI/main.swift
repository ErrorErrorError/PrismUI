...
if let stringURL = CommandLine.arguments.last {
    let url = URL(fileURLWithPath: stringURL)
    convertSSPresetToPrismPreset(url: url)
} else {
    print("Please add link to preset.")
}

func convertSSPresetToPrismPreset(url: URL) {
        do {
...
func createPreset(name: String, values: NSArray, model: String) {
    ...
    let presetsPath = url.appendingPathComponent("\(presetName)-perKey\(modelName.capitalized).bin")
...
func createEffectPreset(values: [UInt8]) -> PrismEffect? {
    ...
    effect.duration = effectLength * 10
